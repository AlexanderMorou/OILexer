    ...
    partial class SimpleLanguageState
    {
        partial class Merger
        {
            internal class NFALookupTable
            {
                ...
                public bool ContainsTransitionFor(SimpleLanguageState[] state, SimpleLanguageBitArray transition)
                {
                    /* *
                ...
                public NFALookup CreateTransitionFor(SimpleLanguageState[] original, SimpleLanguageBitArray transition)
                {
                    SimpleLanguageState dfaState = new SimpleLanguageState();
                ...
                public NFALookup CreateTransitionFor(SimpleLanguageState[] original, SimpleLanguageBitArray transition, SimpleLanguageState dfaState)
                {
                    NFALookup result = new NFALookup(original, dfaState, transition);
                    if (original.Any(p => p.IsMarked))
                        dfaState.MakeEdge();
                ...
                public NFALookup GetTransitionFor(SimpleLanguageState[] state, SimpleLanguageBitArray target)
                {
                    foreach (var lookup in lookups)
                    {
